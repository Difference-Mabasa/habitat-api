...
/**
 * Notifies the agent that the online landlord has approved the
 * mandate. The terminal MANDATE_ACTIVE acknowledgement to both
 * parties is handled by {@link MandateActiveListener} (a separate
 * {@link MandateActiveEvent} fires alongside this one).
 */
@Component
@Slf4j
@RequiredArgsConstructor
public class MandateApprovedListener {
    ...
    public void onMandateApproved(MandateApprovedEvent event) {
        Mandate mandate = mandates.findById(event.mandateId()).orElse(null);
        if (mandate == null) {
            log.warn("mandate {} vanished before approve notifications — skipping",
                    event.mandateId());
            return;
        }
        ...
        tryPush(agent,
                NotificationType.MANDATE_APPROVED,
                NotificationMessages.MANDATE_APPROVED_TITLE,
                TemplateUtils.format(
                        NotificationMessages.MANDATE_APPROVED_BODY,
                ...
                NotificationMessages.MANDATE_APPROVED_ACTION_LABEL,
                mandate.getId());
        log.info("mandate {} approved — agent {} notified", mandate.getId(), agent.getId());
    }
